...
// ─── Webcam Feed ──────────────────────────────────────────────────────────────
// FIX 1: Model load hone se pehle koi value nahi bheji jaayegi (na -1, na 0)
// FIX 2: minConfidence 0.3 rakhi hai taaki multiple faces better detect ho
function WebcamFeed({
  userName,
  onFaceCountChange,
}: {
  ...
  // Kitne consecutive frames mein face nahi dikh raha
  const absentFrames = useRef(0);
  // Kya model load ho chuka hai
  ...
  useEffect(() => {
    ...
    const startCam = async () => {
      try {
        ...
        // tinyFaceDetector load karo — ssdMobilenetv1 se zyada stable hai
        await faceapi.nets.tinyFaceDetector.loadFromUri("/models");
        modelLoadedRef.current = true;

        // Model + video dono ready hone ka wait
        await new Promise((res) => setTimeout(res, 2000));

        // Har 3 second mein detection
        detectionRef.current = setInterval(async () => {
          if (!videoRef.current || !modelLoadedRef.current) return;
          if (videoRef.current.readyState < 2) return;

          let detections: faceapi.FaceDetection[] = [];
          try {
            // tinyFaceDetector use karo — minor movement/lighting pe stable
            detections = await faceapi.detectAllFaces(
              videoRef.current,
              new faceapi.TinyFaceDetectorOptions({ scoreThreshold: 0.4, inputSize: 320 })
            );
          } catch {
            // Detection fail hua — ignore karo, false trigger mat karo
            return;
          }
          ...
          if (count > 1) {
            ...
            // 10 consecutive misses (~30 sec) ke baad hi "0" bhejo
            if (absentFrames.current >= 6) {
              onFaceCountChange?.(0);
            } else {
              onFaceCountChange?.(-1);
            }
...
const Agent = ({
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // RULE 1: Multiple faces detected → turant 1 flag
  // faceCount > 1 aate hi ek baar flag lagao
  // faceCount badalta rahe tab bhi bar bar flag na lage → lastMultiFlagCount
  // ─────────────────────────────────────────────────────────────────────────────
  const lastMultiFlagRef = useRef(false);

  useEffect(() => {
    if (!isActive) return;

    if (faceCount > 1) {
      // Sirf ek baar flag lagao jab multiple face aaye
      if (!lastMultiFlagRef.current) {
        lastMultiFlagRef.current = true;
        setRedFlags((prev) => {
      ...
      // Single face ya no face → reset karo taaki next time phir flag lage
      lastMultiFlagRef.current = false;
    }
  }, [faceCount, isActive]);

  // ─────────────────────────────────────────────────────────────────────────────
  // RULE 2: Face missing → 10 sec countdown → flag
  // faceCount = -1 → ignore (model load ya unstable)
  // faceCount = 0  → countdown start
  // faceCount >= 1 → countdown cancel
  // ─────────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!isActive) return;
    if (faceCount === -1) return; // ignore

    if (faceCount === 0) {
      // Timer already chal raha hai toh dobara mat start karo
          ...
          <div className="w-full max-w-4xl px-4 md:px-0">
            <div className="flex items-center justify-between gap-3 bg-yellow-500/10 border border-yellow-500/50 rounded-2xl px-5 py-3">
              ...
              <span className={cn(
                ...
                missingCountdown <= 3 ? "text-red-500 animate-pulse" : "text-yellow-400"
              )}>
                {missingCountdown}s
              </span>
            </div>
